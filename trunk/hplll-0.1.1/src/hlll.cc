...
Lattice<ZT,FT, MatrixZT, MatrixFT>::hlll(long double delta) { 
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::householder_r(int kappa)
{
  ...
  // R only to nmax, it is entirely recomputed 

  // kappa == 0, we keep nothing 
  // ---------------------------
  if (kappa==0) {
  ...
  // ---------------------------------------------
    ...
    if (col_kept[kappa]) {
      ...
      // Keep the current norm and the floatin point value of B 
  ...
  // -----------------------------------------------------------
    ...
    vector<FP_NR<FT> > toR(nmaxkappa);

    // Dummy in the standard case, made special for the MatrixExp case 
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::householder_v(int kappa) 
{
  ...
  //R.normalize(kappa,nmaxkappa);  // voir si nÈcessaire ? Rajouter en dummy si besoin aussi mpfr 
  ...
  if (w >=0) {
    ...
    fp_norm(s,R.getcol(kappa,kappa),nmaxkappa-kappa); 
    tmpdpe.neg(s);
    R.set(kappa,kappa,tmpdpe);  // On ne met pas ‡ zÈro, inutile, sauf pour getR 
  }
  else {
  ...
  for (i=nmaxkappa; i< n; i++) V.set(i,kappa,0.0);  // V ‡ zÈro car rÈ-utilisÈ plus loin ensuite (pas R); 
  tmpdpe.div(w,s);
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::init(int n, int d, bool forU=false, int gchrono=0) {
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::Lattice(ZZ_mat<ZT> A, bool forU=false, int reduction_method=0, int gchrono=0) {
...
template<class ZT,class FT, class MatrixZT, class MatrixFT>
Lattice<ZT,FT, MatrixZT, MatrixFT>::Lattice(ZZ_mat<ZT> A, long t, long sigma, bool forU=false, int reduction_method=0, int gchrono=0) {
...
template<class ZT,class FT, class MatrixZT, class MatrixFT>  void 
Lattice<ZT,FT, MatrixZT, MatrixFT>::put(ZZ_mat<ZT> A, long t, long sigma) {
...
template<class ZT,class FT, class MatrixZT, class MatrixFT>  void 
Lattice<ZT,FT, MatrixZT, MatrixFT>::shift(ZZ_mat<ZT> A, long m, long sigma) {
...
template<class ZT,class FT, class MatrixZT, class MatrixFT> inline void Lattice<ZT,FT, MatrixZT, MatrixFT>::isreduced(long double deltain) {
