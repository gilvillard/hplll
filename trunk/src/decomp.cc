...
Fgas<RT, ZT, FT, MatrixRT, MatrixZT, MatrixFT>::decomp(long double gamma, long int targetdim, long int targetsize=LONG_MAX) { 
  ...
  while (true) {
    ...
    // Bad computation of the log_2 for the bit size 
    Z_NR<ZT> res;
    set_f(res,tmp1);
    relation_bound = mpz_sizeinbase(res.getData(),2);
    ...
    // Heuristic on the size of U 
      ...
      for (i=0; i<rowU ; i++) 
	for (j=0; j<n; j++)
	  l=max(l, (int) mpz_sizeinbase((U.get(i,j)).getData(),2)); 

      if (((unsigned int) l) > getprec_fgas()) {
...
Fgas<RT, ZT, FT, MatrixRT, MatrixZT, MatrixFT>::hsizereduce(int kappa) { 
  ...
  while (nonstop) {
	...
	/* ----------------------------------------------- */
	  ...
	  vector_addmul(W.getcol(i),W.getcol(kappa),xz,n);
...
inline unsigned int Fgas<RT, ZT, FT, MatrixRT, MatrixZT, MatrixFT>::shift_epsilon(int bits) { 
  ...
  mpfr_set_default_prec(1200); // !!!!!!!
  FP_NR<RT> r1, r2;
  double prec;
  
  // !! bits and oldprec must be int 
  prec=(double) (bits-oldprec); // heuritic to check 
  
  r1=prec;
  r2=2.0;
  r2.log(r2);
  r1.mul(r1,r2);
  epsilon.exponential(r1); 
...
Fgas<RT, ZT, FT, MatrixRT, MatrixZT, MatrixFT>::setprec_internal(long prec) {
  ...
  //mpfr_set_default_prec(getprec_fgas());
...
Fgas<RT, ZT, FT, MatrixRT, MatrixZT, MatrixFT>::init(int d, int n, long setprec, long int inputdec) {
  ...
  prec=(double) n+20-setprec; // heuritic to check 
...
// Construction 
template<class RT, class ZT, class FT, class MatrixRT, class MatrixZT, class MatrixFT>  
Fgas<RT, ZT, FT, MatrixRT, MatrixZT, MatrixFT>::Fgas(MatrixRT Finput, long setprec, long int inputdec=0) {
