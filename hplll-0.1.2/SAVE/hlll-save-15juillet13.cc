...
Lattice<ZT,FT, MatrixZT, MatrixFT>::hlll(double delta) { 
  ...
  // ICI 
  cout << endl <<  "Descend : " << tpsd << " us" << endl;
  cout <<  "Monte : " << tpsm << " us" << endl << endl; 
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::hsizereduce(int kappa) { 
  ...
  // ICI 
  int tt; 
  if  (nblov ==20218) tt=utime();
  while (nonstop) {

    // ICI 
    if (w == 3) {
      cout << endl << "-------------- " << nblov  << endl; 
    ...
    for (i=kappa-1; i>-1; i--){   // Loop through the column 
                                  // -----------------------
  ...
  if (expo == 0) {
    if (lx == 1)
	  ...
	  if (expo == 0) {
	    //long ii;
	    //ii=xz.get_si(); 
	    //B.addmulcol_si(kappa,i,-ii,nmax);
	    B.addmulcol_si(kappa,i,-lx,nmax);
	    //if (transf) {
	    //if (ii > 0) vector_submul_ui(U.getcol(kappa),U.getcol(i),ii,min(d,nmax));
	    //else vector_addmul_ui(U.getcol(kappa),U.getcol(i),-ii,min(d,nmax));
	    //}
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::householder_r(int kappa)
{
  ...
  // if (descendu[kappa]==1)  {
  //cout << endl << "--------------  " << kappa+1 << "   --------------------" << endl;  
  //print2maple(Rkept,n,kappa+1);
...
template<class ZT,class FT, class MatrixZT, class MatrixFT>
Lattice<ZT,FT, MatrixZT, MatrixFT>::Lattice(MatrixRZ<matrix, FP_NR<mpfr_t>, Z_NR<ZT> > A, bool forU=false, int reduction_method=0, int gchrono=0) {
