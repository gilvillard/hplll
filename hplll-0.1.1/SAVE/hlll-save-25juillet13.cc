...
Lattice<ZT,FT, MatrixZT, MatrixFT>::hlll(double delta) { 
  ...
  while (kappa < d) 
	...
	householder_v(0);
      ...
      // kappa has been decreased until the current value 
      // ------------------------------------------------
      if (descendu[kappa]>=1) {
	
	//householder_r(kappa); // Only re-assigns R from Rkept in this case 
  ...
  cout << "--- monte " << tmpm/1000 << " ms" << endl;
  cout << "--- inside " << tmpcompt/1000 << " ms" << endl;
  return 0;
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::hsizereduce(int kappa) { 
  ...
  FP_NR<FT> approx,xn; // xn dummy for debug 
  ...
  while (nonstop) {
    ...
    for (i=kappa-1; i>-1; i--){  
      ...
      //get_round(x, xs, lx, expo, R.get(i,kappa),R.get(i,i));  

      //if (xs !=0) cout << x << "   " << lx << "   " << expo << endl; 
	...
	householder_r(kappa);
      ...
      nonstop = (normB2[kappa] < t);  // Does decrease? If yes continues.    

      //if (normB2[kappa] >=t) 	cout << "************* " << endl; 
    }
    else 
      {
	//cout << "+++++++++++++ " << endl; 
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::householder_r(int kappa)
{
  ...
  // --------
    ...
    else if (col_kept[kappa]) { 
      // Keeps the norm of the column and the fp value of the column, 
      //   and maybe (depending on the intermediary kappa decrease) the dotproducts 
    ...
    if (descendu[kappa]==0) {  // Otherwise will be computed later 
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::init(int n, int d, bool forU=false) {
...
Lattice<ZT,FT, MatrixZT, MatrixFT>::Lattice(ZZ_mat<ZT> A, bool forU=false, int reduction_method=0) {
  ...
  B.resize(n,d+1); //EXT  // Not in init for the mixed matrix case also 
...
template<class ZT,class FT, class MatrixZT, class MatrixFT>
Lattice<ZT,FT, MatrixZT, MatrixFT>::Lattice(MatrixRZ<matrix, FP_NR<mpfr_t>, Z_NR<ZT> > A, bool forU=false, int reduction_method=0) {
...
template<class ZT,class FT, class MatrixZT, class MatrixFT>  void 
Lattice<ZT,FT, MatrixZT, MatrixFT>::put(ZZ_mat<ZT> A, long upperdim, long t, long tau=0) {
...
template<class ZT,class FT, class MatrixZT, class MatrixFT>  void 
Lattice<ZT,FT, MatrixZT, MatrixFT>::mixed_put(MatrixRZ<matrix, FP_NR<mpfr_t>, Z_NR<ZT> > A, long t, long tau=0) {
