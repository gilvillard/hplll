...
class Fgas
{
 ...
 // Input floating point matrix possible concerned with the precision (and possible precision change) 
  ...
  int rowU;   // Possibly not all U is computed, for the precision exhaustion test  
...
public:
  ...
  int shiftU;  // For the stopping criterion on the size of U 
  ...
  void set(MatrixRT Finput, MatrixRT res);

  void set(MatrixRT Finput);
  ...
  unsigned int shift_testU(int bits);
